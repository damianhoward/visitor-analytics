...
package `installDist` once, ship those exact bytes over SSH against a host key pinned from
[`known_hosts.pub`](known_hosts.pub), unpack into `~/releases/visitor-analytics/<commit>`, move
the `~/visitor-analytics` symlink onto it, sync the unit only when it differs, and gate success on
a health check. A release that does not come up is rolled back by the same remote script, so a
runner that dies mid-deploy cannot leave a broken release serving. Three releases are retained.
...
repository. The deploy account needs sudo to install the unit file and manage the service.
