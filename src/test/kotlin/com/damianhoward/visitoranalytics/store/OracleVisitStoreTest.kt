...
class OracleVisitStoreTest {
    ...
    fun `top referrers ranks and bounds`() {
        store.record(sampleVisit(referrer = "https://github.com/damianhoward", at = now.minusSeconds(1)))
        store.record(sampleVisit(referrer = "https://github.com/damianhoward", at = now.minusSeconds(2)))
        store.record(sampleVisit(referrer = "https://linkedin.com", at = now.minusSeconds(3)))

        assertThat(store.topReferrers(1), equalTo(listOf(LabelCount("https://github.com/damianhoward", 2))))
    }
